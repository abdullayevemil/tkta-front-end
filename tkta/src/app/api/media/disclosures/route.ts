...
export async function GET(req: Request) {
  ...
  const disclosures = await sql`SELECT * FROM disclosures
    ${whereClause}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [disclosures] = await sql`
      INSERT INTO disclosures (title, content, date, headerImageUrl, note, titleEnglish, contentEnglish, noteEnglish)
      VALUES (${title}, ${content}, ${date}, ${headerImageUrl}, ${note}, ${titleEnglish}, ${contentEnglish}, ${noteEnglish})
      RETURNING id
    `;

    const disclosuresId = disclosures.id;
    ...
    for (const img of galleryImages) {
      ...
      await sql`
        ...
        VALUES (${disclosuresId}, ${uploaded.secure_url})
      `;
    }

    return NextResponse.json({ success: true, disclosuresId });
  } catch (error) {
