...
export default function News({ params }: Props) {
  ...
  const fetchNews = async () => {
    ...
    const res = await fetch(`/api/announcements/cooperations?${params.toString()}`);
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4 sm:px-8 md:px-16 w-full">
          {news.map((item) => (
            <li key={item.id} className="relative">
              <Link href={`/${locale}/announcements/cooperation/${item.id}`}>
                <NewsCard
                  id={item.id}
                  href={`/${locale}/announcements/cooperation/${item.id}`}
