...
export default function News({ params }: Props) {
  ...
  async function handleDelete() {
    ...
    try {
      const response = await fetch(`/api/media/disclosures/${deleteId}`, {
        method: "DELETE",
      });
      ...
      toast.success(t.media.disclosures.success.delete);
    } catch {
      toast.success(t.media.disclosures.error.delete);
    } finally {
      setOpen(false);
  ...
  const fetchNews = async () => {
    ...
    const res = await fetch(`/api/media/disclosures?${params.toString()}`);

    if (!res.ok) {
      toast.error(t.media.disclosures.error.loading);
        ...
        <div className="flex flex-wrap gap-4">
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {from ? format(from, "dd.MM.yyyy") : t.media.disclosures.from}
              </Button>
            </PopoverTrigger>
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                <CalendarIcon className="mr-2 h-4 w-4" />
                {to ? format(to, "dd.MM.yyyy") : t.media.disclosures.to}
              </Button>
            </PopoverTrigger>
            <PopoverContent align="start" className="w-auto p-0">
              <Calendar
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="new">{t.media.disclosures.newFirst}</SelectItem>
              <SelectItem value="old">{t.media.disclosures.oldFirst}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 px-6 sm:px-10 md:px-24 w-full">
          {news.map((item) => (
            <li key={item.id} className="relative">
              <Link href={`/${locale}/media/disclosures/${item.id}`}>
                <NewsCard
                  id={item.id}
                  href={`/${locale}/media/disclosures/${item.id}`}
                ...
                <div className="absolute top-4 right-4 flex items-center justify-center gap-4">
                  <AlertDialog open={open && deleteId === item.id} onOpenChange={setOpen}>
                    <AlertDialogTrigger asChild>
                      ...
                      >
                        <span>{t.media.disclosures.delete}</span>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>
                          {t.media.disclosures.removeDialogTitle}
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                          {t.media.disclosures.removeDialogContent}
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>
                          {t.media.disclosures.removeDialogCancel}
                        </AlertDialogCancel>
                        <Button variant="destructive" onClick={handleDelete}>
                          {t.media.disclosures.removeDialogConfirm}
                        </Button>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                  <Link
                    href={`/media/disclosures/${item.id}/edit`}
                    className="flex items-center justify-center gap-2 px-2 py-2 bg-blue-600 text-white rounded-md"
                  >
                    <span className="text-sm">{t.media.disclosures.edit}</span>
