...
FROM elementary_school_teaching_program_accreditation
ORDER BY year DESC;
`;
...
export default async function Accreditations({
  ...
  FROM universities u
  JOIN elementary_school_teaching_program_accreditation e
    ...
    <div className="grid grid-cols-4 px-4 md:px-16 gap-6 w-full">
      {universities
        .map((university, index) => (
          <Link
            key={index}
            href={`/reports/accreditation/atm/program/elementary-school-teaching/${year}/${university.toLowerCase()}`}
          >
