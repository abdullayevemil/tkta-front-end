...
export default function News({ params }: Props) {
  ...
  async function handleDelete() {
    ...
    try {
      const response = await fetch(`/api/media/interviews/${deleteId}`, {
        method: "DELETE",
      });
      ...
      toast.success(t.media.interviews.success.delete);
    } catch {
      toast.success(t.media.interviews.error.delete);
    } finally {
      setOpen(false);
  ...
  const fetchNews = async () => {
    ...
    const res = await fetch(`/api/media/interviews?${params.toString()}`);

    if (!res.ok) {
      toast.error(t.media.interviews.error.loading);
    ...
    <div className="w-full flex flex-col gap-8 sm:gap-10 md:gap-12 items-center">
        ...
        <div className="flex flex-wrap gap-4">
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {from ? format(from, "dd.MM.yyyy") : t.media.interviews.from}
              </Button>
            </PopoverTrigger>
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                <CalendarIcon className="mr-2 h-4 w-4" />
                {to ? format(to, "dd.MM.yyyy") : t.media.interviews.to}
              </Button>
            </PopoverTrigger>
            <PopoverContent align="start" className="w-auto p-0">
              <Calendar
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="new">{t.media.interviews.newFirst}</SelectItem>
              <SelectItem value="old">{t.media.interviews.oldFirst}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      {isAdmin && (
        <div className="w-full px-6 sm:px-10 md:px-24">
          ...
          >
            <span>{t.media.interviews.createNews}</span>
          ...
          {news.map((item) => (
            <li key={item.id} className="relative">
              <Link href={`/${locale}/media/interviews/${item.id}`}>
                <NewsCard
                  id={item.id}
                  href={`/${locale}/media/interviews/${item.id}`}
                ...
                <div className="absolute top-4 right-4 flex items-center justify-center gap-4">
                  <AlertDialog open={open && deleteId === item.id} onOpenChange={setOpen}>
                    <AlertDialogTrigger asChild>
                      ...
                      >
                        <span>{t.media.interviews.delete}</span>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>
                          {t.media.interviews.removeDialogTitle}
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                          {t.media.interviews.removeDialogContent}
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>
                          {t.media.interviews.removeDialogCancel}
                        </AlertDialogCancel>
                        <Button variant="destructive" onClick={handleDelete}>
                          {t.media.interviews.removeDialogConfirm}
                        </Button>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                  <Link
                    href={`/media/interviews/${item.id}/edit`}
                    className="flex items-center justify-center gap-2 px-2 py-2 bg-blue-600 text-white rounded-md"
                  >
                    <span className="text-sm">{t.media.interviews.edit}</span>
