...
export async function POST(req: NextRequest) {
  try {
    ...
    const [vacancies] = await sql`
      INSERT INTO vacancies (title, content, date, headerImageUrl, note, titleEnglish, contentEnglish, noteEnglish)
      VALUES (${title}, ${content}, ${date}, ${headerImageUrl}, ${note}, ${titleEnglish}, ${contentEnglish}, ${noteEnglish})
      RETURNING id
    `;

    const vacanciesId = vacancies.id;
    ...
    for (const img of galleryImages) {
      ...
      await sql`
        ...
        VALUES (${vacanciesId}, ${uploaded.secure_url})
      `;
    }

    return NextResponse.json({ success: true, vacanciesId });
  } catch (error) {
