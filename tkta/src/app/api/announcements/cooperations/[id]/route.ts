...
export async function GET(
  ...
  const cooperationsResult = await sql`SELECT * FROM cooperations WHERE id = ${idNumber}`;
  if (cooperationsResult.length === 0) {
    return NextResponse.json({ error: "cooperations not found" }, { status: 404 });
  }

  const cooperations = cooperationsResult[0];

  const photosResult = await sql`
    SELECT id, url 
    FROM cooperations_photos 
...
export async function PUT(
  ...
  await sql`DELETE FROM cooperations_photos WHERE vacancy_id = ${id}`;
  ...
  for (const img of galleryImages) {
    ...
    await sql`
        INSERT INTO cooperations_photos (vacancy_id, url)
...
export async function DELETE(
  ...
  await sql`DELETE FROM cooperations WHERE id = ${idNumber}`;
