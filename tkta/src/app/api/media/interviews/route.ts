...
export async function GET(req: Request) {
  ...
  const interviews = await sql`SELECT * FROM interviews
    ${whereClause}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [interviews] = await sql`
      INSERT INTO interviews (title, content, date, headerImageUrl, note, titleEnglish, contentEnglish, noteEnglish)
      VALUES (${title}, ${content}, ${date}, ${headerImageUrl}, ${note}, ${titleEnglish}, ${contentEnglish}, ${noteEnglish})
      RETURNING id
    `;

    const interviewsId = interviews.id;
    ...
    for (const img of galleryImages) {
      ...
      await sql`
        INSERT INTO interviews_photos (interviews_id, url)
        VALUES (${interviewsId}, ${uploaded.secure_url})
      `;
    }

    return NextResponse.json({ success: true, interviewsId });
  } catch (error) {
