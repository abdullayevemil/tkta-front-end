...
export async function GET(req: Request) {
  ...
  const cooperations = await sql`SELECT * FROM cooperations
    ${whereClause}
  ...
  const count = await sql`
    SELECT COUNT(*) FROM cooperations
    ${whereClause}
  `;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [cooperations] = await sql`
      INSERT INTO cooperations (title, content, date, headerImageUrl, note, titleEnglish, contentEnglish, noteEnglish)
      VALUES (${title}, ${content}, ${date}, ${headerImageUrl}, ${note}, ${titleEnglish}, ${contentEnglish}, ${noteEnglish})
      RETURNING id
    `;

    const cooperationsId = cooperations.id;
    ...
    for (const img of galleryImages) {
      ...
      await sql`
        ...
        VALUES (${cooperationsId}, ${uploaded.secure_url})
      `;
    }

    return NextResponse.json({ success: true, cooperationsId });
  } catch (error) {
