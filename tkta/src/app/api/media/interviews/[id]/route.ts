...
export async function GET(
  ...
  const interviewsResult = await sql`SELECT * FROM interviews WHERE id = ${idNumber}`;
  if (interviewsResult.length === 0) {
  ...
  const photosResult = await sql`
    ...
    FROM interviews_photos 
    WHERE interviews_id = ${idNumber}
  `;
...
export async function PUT(
  ...
  await sql`DELETE FROM interviews_photos WHERE interviews_id = ${id}`;
...
export async function DELETE(
  ...
  await sql`DELETE FROM interviews WHERE id = ${idNumber}`;
